...
* 首页主题切换：点击底部色条，切换 EVA-00/01/02
* 文章页预览：点击首页文章卡片进入详情页
* 后台管理：访问 `/login` → 输入管理员账号密码
* 编辑器功能：在后台文章编辑区插入链接、图片、字号、颜色、对齐等

### ✅ 常见问题（本地）

* 端口冲突：修改 `PORT` 环境变量或关闭占用 3000 端口的程序
* 依赖错误：删除 `node_modules` 后重新 `npm install`
